...
def create_lastfm_wordcloud(
    user, timeframe, artists_count, top_tags_count, forbidden_tags
):
    dict_frequencies = {}
...
def main():
    ...
    for user in users:
        ...
        wordcloud = create_lastfm_wordcloud(
            lastfm_user,
            args.timeframe,
            args.artists_count,
            args.top_tags_count,
        ...
        with open(f"Exports/{user}_{int(time.time())}_{args.timeframe}.png", "wb") as f:
            f.write(wordcloud.getbuffer())
...
def parse_args():
    ...
    parser.add_argument(
        "--artists_count",
        "-a",
        help="Number of artists to extract tags from (default 50).",
        type=int,
        default=50,
    )
    parser.add_argument(
